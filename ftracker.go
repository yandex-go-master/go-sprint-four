...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	meanSpeedMetersInSec := meanSpeed(action, duration) * kmhInMsec
	return (walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeedMetersInSec, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
